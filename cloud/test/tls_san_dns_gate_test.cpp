...
class ScopedCaPath {
public:
    explicit ScopedCaPath(std::string value) : old_value_(config::tls_ca_certificate_path) {
        config::tls_ca_certificate_path = std::move(value);
    }

    ~ScopedCaPath() { config::tls_ca_certificate_path = old_value_; }
...
TEST_F(TlsSanDnsGateTest, ParseConfigIgnoresInvalidSegments) {
    auto parsed = TlsSanDnsGate::parse_config("bad;http=internal.com;brpc= ;brpc=internal.com");
...
TEST_F(TlsSanDnsGateTest, ParseConfigAcceptsIpLikeDns) {
    auto parsed = TlsSanDnsGate::parse_config("brpc=10.23.45.67.");
...
TEST_F(TlsSanDnsGateTest, ParseConfigThriftSegmentDoesNotAffectBrpcGate) {
    auto parsed = TlsSanDnsGate::parse_config("thrift=internal.com");
    EXPECT_TRUE(parsed.allowed.empty());

    ScopedSanDnsConfig cfg("thrift=internal.com");
    EXPECT_FALSE(TlsSanDnsGate::is_protocol_enabled(TlsSanDnsGate::kProtocolBrpc));
    EXPECT_EQ(TlsSanDnsGate::get_allowed_dns(TlsSanDnsGate::kProtocolBrpc), nullptr);
}
...
TEST_F(TlsSanDnsGateTest, ConfigBackedProtocolLookupRefreshesCache) {
    {
        ...
        const auto* allowed = TlsSanDnsGate::get_allowed_dns(TlsSanDnsGate::kProtocolBrpc);
        ASSERT_NE(allowed, nullptr);
        EXPECT_EQ(allowed->count("internal.com"), 1);
    }

    {
        ScopedSanDnsConfig cfg("brpc=   ");
        EXPECT_FALSE(TlsSanDnsGate::is_protocol_enabled(TlsSanDnsGate::kProtocolBrpc));
        EXPECT_EQ(TlsSanDnsGate::get_allowed_dns(TlsSanDnsGate::kProtocolBrpc), nullptr);
    }
}
...
TEST_F(TlsSanDnsGateTest, VerifyCallbackCoversAllGatePaths) {
    EXPECT_EQ(tls_san_dns_verify_callback(0, nullptr, nullptr), 0);
    EXPECT_EQ(tls_san_dns_verify_callback(1, nullptr, nullptr), 1);
    ...
    std::unique_ptr<X509_STORE_CTX, decltype(&X509_STORE_CTX_free)> store_ctx(X509_STORE_CTX_new(),
                                                                              X509_STORE_CTX_free);
    ASSERT_NE(store_ctx, nullptr);
    ASSERT_EQ(X509_STORE_CTX_init(store_ctx.get(), store.get(), cert.get(), nullptr), 1);
    X509_STORE_CTX_set_current_cert(store_ctx.get(), cert.get());

    const std::string ca_path = "/tmp/cloud_tls_san_dns_gate_test_ca.crt";
    std::ofstream ca_file(ca_path);
    ASSERT_TRUE(ca_file.is_open());
    ca_file << kTestCertWithSan;
    ca_file.close();
    ScopedCaPath ca_cfg(ca_path);

    X509_STORE_CTX_set_error_depth(store_ctx.get(), 1);
    EXPECT_EQ(tls_san_dns_verify_callback(1, store_ctx.get(), nullptr), 1);
    X509_STORE_CTX_set_error_depth(store_ctx.get(), 0);

    {
        ScopedSanDnsConfig cfg("");
        EXPECT_EQ(tls_san_dns_verify_callback(1, store_ctx.get(), nullptr), 1);
    }

    {
        ScopedSanDnsConfig cfg("brpc=other.example.com");
        X509_STORE_CTX_set_error(store_ctx.get(), X509_V_OK);
        EXPECT_EQ(tls_san_dns_verify_callback(1, store_ctx.get(), const_cast<char*>("")), 0);
        EXPECT_EQ(X509_STORE_CTX_get_error(store_ctx.get()), X509_V_ERR_APPLICATION_VERIFICATION);
    }

    {
        ScopedSanDnsConfig cfg("brpc=testclient.example.com");
        X509_STORE_CTX_set_error(store_ctx.get(), X509_V_OK);
        EXPECT_EQ(tls_san_dns_verify_callback(1, store_ctx.get(),

...
suite("test_search_lucene_mode") {
    ...
    // Wait for index building (increased from 3s to 5s for more reliable phrase queries)
    Thread.sleep(5000)
    ...
    // with minimum_should_match=1. This follows ES/Lucene semantics where a pure negative
    // query (only MUST_NOT, no MUST/SHOULD) returns all documents EXCEPT those matching
    // the NOT clause. The MATCH_ALL_DOCS acts as a positive clause that matches all docs.
    // Expected: all rows except those containing 'apple' -> 4, 5, 6, 7
    qt_lucene_not """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
    ...
    // ============ Test 18: Lucene mode pure NOT with phrase query ============
    // 'NOT "apple banana"' in Lucene mode is rewritten to: SHOULD(MATCH_ALL_DOCS) + MUST_NOT(phrase)
    // This tests the original bug scenario: select count(*) from t where search('NOT "term"', options)
    // Expected: all rows except those containing exact phrase "apple banana" -> 3, 4, 5, 6, 7, 8
    qt_lucene_not_phrase """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('NOT "apple banana"', '{"default_field":"title","default_operator":"and","mode":"lucene"}')
        ORDER BY id
    """

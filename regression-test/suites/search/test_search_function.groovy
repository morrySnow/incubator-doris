...
suite("test_search_function") {
    ...
    try {
        ...
        // After the fix for variant subcolumn search, the error message comes from
        // evaluate_inverted_index() which now gets properly called
        assertTrue(e.getMessage().contains("search() requires inverted indexes"))
    }
}

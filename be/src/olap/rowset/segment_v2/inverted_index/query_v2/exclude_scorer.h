...
inline bool is_within(TDocSetExclude& docset, uint32_t doc) {
    uint32_t current = docset->doc();
    // Optimization: if current is past doc (but not TERMINATED), skip seek
    // TERMINATED means "not positioned" or "exhausted", so we must seek to check
    if (current != TERMINATED && current > doc) {
        return false;
    }
    return docset->seek(doc) == doc;
}

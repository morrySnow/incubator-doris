...
public:
    ...
    explicit SegmentPostings(TermDocsPtr iter) : _iter(std::move(iter)) {
    ...
    explicit SegmentPostings(TermPositionsPtr iter) : _iter(std::move(iter)), _has_positions(true) {
    ...
    uint32_t norm() const override {
        // NOTE: hubspot CLucene DocRange doesn't have norm_many
        // Always return 1 (no scoring support)
        return 1;
    }
...
inline SegmentPostingsPtr make_segment_postings(TermDocsPtr iter) {
    return std::make_shared<SegmentPostings>(std::move(iter));
}

...
ScorerPtr PhraseScorer<TPostings>::create_with_offset(
        ...
        uint32_t slop, size_t offset, uint32_t num_docs) {
...
uint32_t PhraseScorer<TPostings>::seek(uint32_t target) {
    ...
    // Note: We must NOT return early when doc() is TERMINATED (not positioned or exhausted),
    // because any target < TERMINATED would incorrectly trigger the early return.
    if (current_doc != TERMINATED && target <= current_doc) {
        return current_doc;
    }
    uint32_t result_doc = _intersection_docset->seek(target);
    if (result_doc == TERMINATED || phrase_match()) {
        return result_doc;
    }
    return advance();
}
...
float PhraseScorer<TPostings>::score() {
    // Scoring disabled - always return 1.0
    return 1.0F;
}

template <typename TPostings>
bool PhraseScorer<TPostings>::phrase_match() {
    // Scoring disabled - just check if phrase exists

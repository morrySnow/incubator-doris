...
public:
    ...
    ScorerPtr scorer(const QueryExecutionContext& context) override {
        // AllScorer(0) behaves like EmptyScorer - returns TERMINATED immediately
        return std::make_shared<AllScorer>(context.segment_num_rows);
    }

    ScorerPtr scorer(const QueryExecutionContext& context,
                     const std::string& binding_key) override {
        auto all_scorer = std::make_shared<AllScorer>(context.segment_num_rows);
        if (_nullable) {
            auto logical_field = logical_field_or_fallback(context, binding_key, _field);
            return make_nullable_scorer(all_scorer, logical_field, context.null_resolver);
        }
        return all_scorer;
    }
...
public:
    AllQuery() = default;
    AllQuery(std::wstring field, bool nullable)
            : _field(std::move(field)), _nullable(nullable) {}
    ~AllQuery() override = default;

    WeightPtr weight() override { return std::make_shared<AllWeight>(_field, _nullable); }

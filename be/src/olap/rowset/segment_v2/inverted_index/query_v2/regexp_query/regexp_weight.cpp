...
RegexpWeight::RegexpWeight(IndexQueryContextPtr context, std::wstring field, std::string pattern,
        ...
        : _context(std::move(context)),
          ...
          _pattern(normalize_pattern(std::move(pattern))),
          _nullable(nullable) {
    // _max_expansions = _context->runtime_state->query_options().inverted_index_max_expansions;
}

std::string RegexpWeight::normalize_pattern(std::string pattern) {
    // Lucene regex matches against the entire term, but Hyperscan does substring matching.
    // Add anchors to enforce whole-term matching for Lucene compatibility.
    // Only add anchors if they're not already present.
    if (pattern.empty()) {
        return "^$";
    }
    bool has_start_anchor = pattern[0] == '^';
    bool has_end_anchor = pattern[pattern.size() - 1] == '$';

...
public:
    ...
    WeightPtr weight() override {
        ...
        for (const auto& [occur, query] : _sub_queries) {
            sub_weights.emplace_back(occur, query->weight());
        }
        auto combiner = std::make_shared<DoNothingCombiner>();
        return std::make_shared<OccurBooleanWeight<DoNothingCombinerPtr>>(
                std::move(sub_weights), _minimum_number_should_match, combiner);
    }

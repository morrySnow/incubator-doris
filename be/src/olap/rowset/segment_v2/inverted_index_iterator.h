...
public:
    // New interface constructor (for cherry-picked code)
    InvertedIndexIterator();

    // Old interface constructor (for backward compatibility with hubspot code)
    ...
    // New interface (cherry-picked code)
    Status read_from_index(const IndexParam& param) override;

    // Old interface (backward compatibility) - for hubspot code
...
private:
    ...
    // Old interface member variables (for backward compatibility)

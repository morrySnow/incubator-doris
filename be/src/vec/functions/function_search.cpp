...
Status FieldReaderResolver::resolve(const std::string& field_name,
    ...
    if (!reader_result.has_value()) {
        // Index metadata exists but reader selection failed
        // For variant subcolumns, return empty binding (no match) instead of error
        if (is_variant_sub) {
            VLOG_DEBUG << "Failed to select inverted index reader for variant subcolumn '"
                       << field_name << "', returning empty result.";
            *binding = FieldReaderBinding();
            return Status::OK();
        }
    ...
    if (inverted_reader == nullptr) {
        // For variant subcolumns, return empty binding instead of error
        if (is_variant_sub) {
            VLOG_DEBUG << "Inverted index reader is null for variant subcolumn '" << field_name
    ...
    if (index_file_reader == nullptr) {
        // For variant subcolumns, return empty binding instead of error
        if (is_variant_sub) {
            VLOG_DEBUG << "Index file reader is null for variant subcolumn '" << field_name
                       << "', returning empty result.";
            *binding = FieldReaderBinding();
            return Status::OK();
        }
    ...
    if (auto st = index_file_reader->init(config::inverted_index_read_buffer_size, _context->io_ctx);
        !st.ok()) {
        if (is_variant_sub) {
            VLOG_DEBUG << "Failed to init index file reader for variant subcolumn '" << field_name
                       << "', returning empty result. Error: " << st.msg();
            *binding = FieldReaderBinding();
            return Status::OK();
        }
        return st;
    ...
    if (!directory_result.has_value()) {
        if (is_variant_sub) {
            VLOG_DEBUG << "Failed to open index file for variant subcolumn '" << field_name
                       << "', returning empty result. Error: " << directory_result.error().msg();
            *binding = FieldReaderBinding();
            return Status::OK();
        }
    ...
    // Store the directory to keep it alive while IndexReaders are in use.
    // The IndexReader holds raw pointers to the stream inside DorisCompoundReader,
    // so we must ensure DorisCompoundReader outlives the IndexReader.
    _directories.push_back(std::move(directory));
    auto* directory_ptr = _directories.back().get();

    lucene::index::IndexReader* raw_reader = nullptr;
    try {
        raw_reader = lucene::index::IndexReader::open(
                directory_ptr, config::inverted_index_read_buffer_size, false);
    } catch (const CLuceneError& e) {
        // For variant subcolumns, this typically means:
        // 1. Subcolumn has mixed types (string + number) so inverted index wasn't built
        // 2. Subcolumn values are all numeric types which don't support text search
        // Return empty binding to signal "no match" instead of error - this allows the query
        // to return 0 rows gracefully rather than failing.
        if (is_variant_sub) {
            VLOG_DEBUG << "Failed to open inverted index for variant subcolumn '" << field_name
                       << "', returning empty result. Original error: " << e.what();
            *binding = FieldReaderBinding();
            return Status::OK();
        }
    ...
    if (raw_reader == nullptr) {
        // For variant subcolumns, return empty binding instead of error
        if (is_variant_sub) {
            VLOG_DEBUG << "IndexReader is null for variant subcolumn '" << field_name
                       << "', returning empty result.";
            *binding = FieldReaderBinding();
            return Status::OK();
        }
...
Status FunctionSearch::evaluate_inverted_index_with_search_param(
        ...
        std::unordered_map<std::string, segment_v2::InvertedIndexIterator*> iterators, uint32_t num_rows,
        segment_v2::InvertedIndexResultBitmap& bitmap_result) const {
    ...
    // Extract default_operator from search_param (defaults to "or" if not set)
    ...
    // Extract minimum_should_match from search_param (-1 means not set)
    ...
    auto weight = root_query->weight();
    if (!weight) {
        return Status::InternalError("search: Failed to build query weight for DSL");
    }

    auto scorer = weight->scorer(exec_ctx, root_binding_key);
    if (!scorer) {
        return Status::InternalError("search: Failed to build scorer for DSL");
    }
    ...
    *out = nullptr;
    ...
    if (clause_type == "TERM") {
        ...
        if (should_analyze) {
            ...
            // - default_operator=AND + msm: MUST clauses, msm on 0 SHOULD → likely 0 results
            // - default_operator=OR + msm: SHOULD clauses, msm controls how many match
            if (minimum_should_match > 0) {
                ...
                // Use MUST for AND, SHOULD for OR (matching ES behavior)
    ...
    if (category == FunctionSearch::ClauseTypeCategory::TOKENIZED) {
        ...
        if (clause_type == "MATCH") {
            return Status::NotSupported(
                    "search: MATCH clause type is not yet implemented. "
                    "Consider using TERM for exact match or PHRASE for phrase matching.");
        }
    ...
    if (category == FunctionSearch::ClauseTypeCategory::NON_TOKENIZED) {
        ...
        if (clause_type == "WILDCARD") {
            // Special case: "*" alone is an AllQuery with nullable=true (matches all non-null values)
            // This is consistent with Elasticsearch's query_string behavior where
            // "*" is converted to FieldExistsQuery instead of matching all terms.
            if (value == "*") {
        ...
        if (clause_type == "RANGE") {
            return Status::NotSupported(
                    "search: RANGE clause type is not yet implemented. "
                    "Use standard SQL range predicates (e.g., col >= 10 AND col <= 20).");
        }
        if (clause_type == "LIST") {
            return Status::NotSupported(
                    "search: LIST clause type is not yet implemented. "
                    "Use standard SQL IN clause (e.g., col IN ('a', 'b', 'c')).");
        }
    ...
    return Status::NotSupported(
            "search: Unknown clause type '{}'. "
            "Supported types: TERM, PHRASE, ANY, ALL, EXACT, PREFIX, WILDCARD, REGEXP.",
            clause_type);
}

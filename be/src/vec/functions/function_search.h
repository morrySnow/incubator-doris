...
public:
    ...
    // A field is considered a variant subcolumn if:
    // 1. It's explicitly marked in field_bindings, OR
    // 2. The field name contains '.' (e.g., "ch.a", "data.user.name")
    //    This matches FE's logic in SearchPredicate.java
    bool is_variant_subcolumn(const std::string& field_name) const {
        // Check explicit marking first
        if (_variant_subcolumn_fields.count(field_name) > 0) {
            return true;
        }
        // Fallback: check if field name contains '.' (like FE does)
    ...
    segment_v2::InvertedIndexIterator* get_iterator(const std::string& field_name) const {
...
private:
    ...
    // AFTER the IndexReaders. The IndexReader holds raw pointers to the stream inside
    // DorisCompoundReader, so DorisCompoundReader must outlive the IndexReader.
    std::vector<std::unique_ptr<segment_v2::DorisCompoundReader>> _directories;
...
public:
    ...
    Status build_query_recursive(const TSearchClause& clause,
                                 ...
                                 int32_t minimum_should_match = -1) const;

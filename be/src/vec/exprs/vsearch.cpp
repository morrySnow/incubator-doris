...
struct SearchInputBundle {
    // NOTE: hubspot branch uses InvertedIndexIterator* instead of IndexIterator*
    std::unordered_map<std::string, segment_v2::InvertedIndexIterator*> iterators;
    ...
    for (const auto& child : expr.children()) {
        if (child->is_slot_ref()) {
            ...
            if (child_index < field_bindings.size()) {
                ...
                // Fallback to expr_name if binding not found
                // NOTE: hubspot branch - VSlotRef doesn't have column_name(), use expr_name()
                field_name = column_slot_ref->expr_name();
            }
...
bool all_variant_subcolumns(const std::vector<TSearchFieldBinding>& field_bindings) {
    if (field_bindings.empty()) {
        return false;
    }
    for (const auto& binding : field_bindings) {
...
Status VSearchExpr::evaluate_inverted_index(VExprContext* context, uint32_t segment_num_rows) {
    VLOG_DEBUG << "VSearchExpr::evaluate_inverted_index called, DSL: " << _search_param.original_dsl;
    ...
    if (bundle.iterators.empty() && !all_variant_subcolumns(_search_param.field_bindings)) {
        return Status::InvalidArgument(
                "search() requires inverted indexes on the referenced columns. "
                "Please create inverted indexes using: ALTER TABLE ... ADD INDEX ... USING INVERTED. "
                "DSL: {}",
                _original_dsl);
    }

...
public:
    ...
    int32_t read(int32_t*, int32_t*, int32_t*, int32_t) { return 0; }
    ...
    bool readRange(DocRange* docRange) override {
        ...
        // SCORING DISABLED: docRange->norm_many = &_norms;
        docRange->doc_many_size_ = static_cast<uint32_t>(_docs.size());
        docRange->freq_many_size_ = static_cast<uint32_t>(_freqs.size());
        // SCORING DISABLED: docRange->norm_many_size_ = static_cast<uint32_t>(_norms.size());
        _read_done = true;
        return true;
    }
...
// SCORING DISABLED: Test skipped - append_positions behavior changed with scoring removal
...
TEST_F(SegmentPostingsTest, test_no_score_segment_posting) {
    ...
    // SCORING DISABLED: freq() returns actual frequency from freq_many, not 1
    EXPECT_EQ(posting.freq(), 5);

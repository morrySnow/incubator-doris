...
static Status boolean_query_search(
    ...
    // SCORING DISABLED: context->collection_statistics = std::make_shared<CollectionStatistics>();
    // SCORING DISABLED: context->collection_similarity = std::make_shared<CollectionSimilarity>();
...
TEST_F(BooleanQueryTest, test_boolean_query_scoring_or) {
    ...
    // 直接访问成员填充统计信息
    // SCORING DISABLED: context->collection_statistics->_total_num_docs = 80;
    // SCORING DISABLED: context->collection_statistics->_total_num_tokens[ws_field] = 240; // 80*3
    // SCORING DISABLED: auto set_df = [&](const std::string& term, uint64_t df) {
    //     context->collection_statistics->_term_doc_freqs[ws_field][StringHelper::to_wstring(term)] = df;
    // };
    // SCORING DISABLED: set_df("apple", 40);
    // SCORING DISABLED: set_df("banana", 40);
    // SCORING DISABLED: set_df("orange", 40);
    // SCORING DISABLED: set_df("cherry", 40);
    // SCORING DISABLED: set_df("grape", 30);
    // SCORING DISABLED: set_df("kiwi", 30);
    // SCORING DISABLED: set_df("strawberry", 20);
    ...
    EXPECT_EQ(count, 50);
    // SCORING DISABLED: In hubspot branch, score() always returns 1.0F
    // All documents have the same score, so we just verify scores are positive
    EXPECT_GT(score_single, 0.0F);
    EXPECT_GT(score_both, 0.0F);
    // SCORING DISABLED: Cannot verify score_both > score_single since scoring is disabled

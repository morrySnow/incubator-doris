...
// SCORING DISABLED: Test modified - scoring functionality removed in hubspot branch
// score() returns 0 (DoNothingCombiner) or 1.0F depending on implementation
TEST_F(OccurBooleanQueryTest, ScoringEnabled) {
    ...
    while (doc != TERMINATED) {
        // SCORING DISABLED: Just verify we can call score() without crashing
        // Actual score value doesn't matter when scoring is disabled
...
class AllQueryMockWeight : public Weight {
public:
    explicit AllQueryMockWeight(uint32_t max_doc) : _max_doc(max_doc) {}
...
class AllQueryMock : public Query {
public:
...
TEST_F(OccurBooleanQueryTest, MustAllQueryWithMustTerm) {
    _ctx.segment_num_rows = 1000;
    auto term_docs = generate_range_docs(0, 500, 2);

    std::vector<std::pair<Occur, QueryPtr>> clauses;
    clauses.emplace_back(Occur::MUST, std::make_shared<AllQueryMock>(1000));
    ...
    EXPECT_EQ(result, term_docs);
}

TEST_F(OccurBooleanQueryTest, SingleMustAllQuery) {
    ...
    auto expected = generate_range_docs(0, 100);
    EXPECT_EQ(result, expected);
}

TEST_F(OccurBooleanQueryTest, MultipleMustAllQueries) {
    _ctx.segment_num_rows = 100;
...
TEST_F(OccurBooleanQueryTest, ShouldAllQueryOrTerm) {
    _ctx.segment_num_rows = 100;
    auto term_docs = generate_range_docs(0, 50, 2);
...
TEST_F(OccurBooleanQueryTest, SingleShouldAllQuery) {
    _ctx.segment_num_rows = 100;
...
TEST_F(OccurBooleanQueryTest, MustAllQueryWithShouldTerm) {
    _ctx.segment_num_rows = 100;
    auto should_docs = generate_range_docs(0, 50);
...
TEST_F(OccurBooleanQueryTest, MustAllQueryWithMustNotTerm) {
    _ctx.segment_num_rows = 100;
    auto must_not_docs = generate_range_docs(0, 50);
    ...
    auto expected = generate_range_docs(50, 100);
    EXPECT_EQ(result.size(), expected.size());
    EXPECT_EQ(to_set(result), to_set(expected));
}

TEST_F(OccurBooleanQueryTest, MustAllQueryMustTermShouldTerm) {
    _ctx.segment_num_rows = 1000;
    auto must_docs = generate_range_docs(0, 500);
    auto should_docs = generate_range_docs(250, 750);
...
TEST_F(OccurBooleanQueryTest, MustNotAllQueryReturnsEmpty) {
    _ctx.segment_num_rows = 100;
    auto must_docs = generate_range_docs(0, 100);
...
TEST_F(OccurBooleanQueryTest, ShouldAllQueryOrTermWithScoring) {
    _ctx.segment_num_rows = 100;
    auto term_docs = generate_range_docs(0, 50, 2);
...
TEST_F(OccurBooleanQueryTest, MustAllQueryWithShouldTermScoring) {
    _ctx.segment_num_rows = 100;
    auto should_docs = generate_range_docs(0, 50);
